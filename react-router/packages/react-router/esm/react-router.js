...
function (_React$Component) {
  ...
  function Router(props) {
    ...
    _this = _React$Component.call(this, props) || this; // debugger
...
function Redirect(_ref) {
  ...
  return React.createElement(context.Consumer, null, function (context) {
    ...
    var history = context.history,
        staticContext = context.staticContext; // 两种方式
    // push,replace

    var method = push ? history.push : history.replace; // 新建的location
...
var cacheCount$1 = 0; // 编译路径、将路径转换成正则表达式
// 并缓存
...
function matchPath(pathname, options) {
  ...
  var _options = options,
      ...
      sensitive = _options$sensitive === void 0 ? false : _options$sensitive; // 为什么使用数组形式？
...
function (_React$Component) {
  ...
  _proto.render = function render() {
    ...
    return React.createElement(context.Consumer, null, function (context$1) {
      !context$1 ? process.env.NODE_ENV !== "production" ? invariant(false, "You should not use <Route> outside a <Router>") : invariant(false) : void 0; // context.location表示当前浏览器实际路径
      // location 为Route自定义匹配路径，用于不匹配当前浏览器实际路径的另一种选择

      var location = _this.props.location || context$1.location; // 1. Route对象置于Switch组件内部，由Switch组件注入computedMatch属性
      // 2. 自身提供path属性，则检查自身匹配性
      // 3. 使用默认路径匹配结果

      var match = _this.props.computedMatch ? _this.props.computedMatch // <Switch> already computed the match for us
      // 是否自身提供了PATH属性，如果没有那么则默认匹配根路径
      : _this.props.path ? matchPath(location.pathname, _this.props) : context$1.match; // context对象
      // history: this.props.history,
      // location: this.state.location,
      // // 根路径匹配
      // match: Router.computeRootMatch(this.state.location.pathname),
      ...
      var _this$props = _this.props,
          ...
          render = _this$props.render; // 兼容preact
