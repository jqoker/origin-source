...
God.executeApp = function executeApp(env, cb) {
  ...
  env_copy["vizion_running"] =
    env_copy["vizion_running"] !== undefined
      ? env_copy["vizion_running"]
      : false;
  ...
  if (env_copy["pm_id"] === undefined) {
    ...
    env_copy.pm_pid_path = env_copy.pm_pid_path.replace(
      /-[0-9]+\.pid$|\.pid$/g,
      "-" + env_copy["pm_id"] + ".pid"
    );
    ...
    if (!env_copy["merge_logs"]) {
      ["", "_out", "_err"].forEach(function (k) {
        ...
        env_copy[key] &&
          (env_copy[key] = env_copy[key].replace(
            /-[0-9]+\.log$|\.log$/g,
            "-" + env_copy["pm_id"] + ".log"
          ));
      });
...
God.handleExit = function handleExit(clu, exit_code, kill_signal) {
  console.log(
    `App [${clu.pm2_env.name}:${
      clu.pm2_env.pm_id
    }] exited with code [${exit_code}] via signal [${kill_signal || "SIGINT"}]`
  );
  ...
  var min_uptime =
    typeof proc.pm2_env.min_uptime !== "undefined"
      ? proc.pm2_env.min_uptime
      : 1000;
  var max_restarts =
    typeof proc.pm2_env.max_restarts !== "undefined"
      ? proc.pm2_env.max_restarts
      : 16;
  ...
  if (
    proc.pm2_env.restart_delay !== undefined &&
  ...
  if (
    proc.pm2_env.exp_backoff_restart_delay !== undefined &&
    ...
    if (!proc.pm2_env.prev_restart_delay) {
      ...
      proc.pm2_env.prev_restart_delay = Math.floor(
        Math.min(15000, proc.pm2_env.prev_restart_delay * 1.5)
      );
      restart_delay = proc.pm2_env.prev_restart_delay;
    }
    console.log(
      `App [${clu.pm2_env.name}:${clu.pm2_env.pm_id}] will restart in ${restart_delay}ms`
    );
  }
...
God.finalizeProcedure = function finalizeProcedure(proc) {
  var last_path = "";
  var current_path =
    proc.pm2_env.cwd || path.dirname(proc.pm2_env.pm_exec_path);
  var proc_id = proc.pm2_env.pm_id;

  proc.pm2_env.version = Utility.findPackageVersion(
    proc.pm2_env.pm_exec_path || proc.pm2_env.cwd
  );

  if (proc.pm2_env.vizion_running === true) {
    debug(
      "Vizion is already running for proc id: %d, skipping this round",
      proc_id
    );
...
God.injectVariables = function injectVariables(env, cb) {
  ...
  var instanceNumber =
    typeof instances[0] === "undefined" ? 0 : instances[0] + 1;
  ...
  if (env.increment_var) {
    ...
    env[env.increment_var] =
      typeof lastIncrement === "undefined" ? defaut : lastIncrement + 1;
