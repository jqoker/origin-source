...
class API {
    ...
    if (semver.lt(process.version, "6.0.0")) {
      Common.printOut(
        conf.PREFIX_MSG_WARNING +
          "Node 4 is deprecated, please upgrade to use pm2 to have all features"
      );
    }
  ...
  reset(process_name, cb) {
    ...
    function processIds(ids, cb) {
      eachLimit(
        ...
        function (id, next) {
          that.Client.executeRemote("resetMetaProcessId", id, function (
            err,
            res
          ) {
    ...
    that.getVersion(function (err, new_version) {
      ...
      that.dump(function (err) {
        ...
        that.killDaemon(function () {
          ...
          that.Client.launchDaemon({ interactor: false }, function (
            err,
            child
          ) {
  ...
  reload(process_name, opts, cb) {
    ...
    if (Common.isConfigFile(process_name))
      that._startJson(process_name, opts, "reloadProcessId", function (
        err,
        apps
      ) {
        Common.unlockReload();
      ...
      that._operate("reloadProcessId", process_name, opts, function (
        err,
        apps
      ) {
        Common.unlockReload();
    ...
    function restartExistingNameSpace(cb) {
      ...
      if (script !== "all") {
        that.Client.getProcessIdsByNamespace(script, function (err, ids) {
          if (err && cb) return cb(err);
          if (ids.length > 0) {
            that._operate("restartProcessId", script, opts, function (
              err,
              list
            ) {
              if (err) return cb(err);
    ...
    function restartExistingProcessPathOrStartNew(cb) {
      that.Client.executeRemote("getMonitorData", {}, function (err, procs) {
        ...
        if (
          ...
          that._operate("restartProcessId", app_name, opts, function (
            err,
            list
          ) {
            if (err)
        ...
        that.Client.executeRemote("prepare", resolved_paths, function (
          err,
          data
        ) {
          if (err) {
  ...
  _startJson(file, opts, action, pipe, cb) {
    ...
    appConf.forEach(function (app) {
      ...
      // Namespace
      if (!opts.namespace && !app.namespace) app.namespace = "default";
      // --watch
    ...
    that.Client.executeRemote("getMonitorData", {}, function (
      err,
      raw_proc_list
    ) {
      if (err) {
        ...
        function (id, next) {
          ...
          if (
            ...
            if (update_env === true) {
              ...
              else new_env = util._extend({}, process.env);
        ...
        function (err, ids) {
          ...
          that.Client.getProcessIdsByNamespace(
            ...
            function (err, ns_process_ids) {
               ...
               * if yes load configuration variables and merge with the current environment
               */
              var ns_additional_env = Modularizer.getAdditionalConf(
                process_name
              );
      ...
      if (
        ...
        that.Client.executeRemote("getMonitorData", {}, (err, proc_list) => {
          ...
          that.Client.getProcessIdByName(process_name, function (err, ids) {
            ...
            that.Client.getProcessIdsByNamespace(process_name, function (
              err,
              ns_process_ids
            ) {
