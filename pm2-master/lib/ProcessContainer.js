...
(function ProcessContainer() {
  ...
  if (
    typeof process.env.source_map_support != "undefined" &&
  ...
  if (cst.MODIFY_REQUIRE) require.main.filename = pm2_env.pm_exec_path;
...
function exec(script, stds) {
  ...
  if (pm2_env.log_date_format) dateFns = require("date-fns");
  ...
  Utility.startLogging(stds, function (err) {
        ...
        if (pm2_env.log_type && pm2_env.log_type === "json") {
          log_data =
            JSON.stringify({
              ...
              timestamp:
                pm2_env.log_date_format && dateFns
                  ? dateFns.format(Date.now(), pm2_env.log_date_format)
          ...
          log_data = `${dateFns.format(
            Date.now(),
            pm2_env.log_date_format
          )}: ${string.toString()}`;
    ...
    process.on(
      "uncaughtException",
      getUncaughtExceptionListener("uncaughtException")
    );
    process.on(
      "unhandledRejection",
      getUncaughtExceptionListener("unhandledRejection")
    );
    ...
    function logError(types, error) {
      try {
        types.forEach(function (type) {
          stds[type] &&
            typeof stds[type].write == "function" &&
