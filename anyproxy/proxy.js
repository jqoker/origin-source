      ...
      [
        ...
        function(callback) {
          if (self.proxyType === T_TYPE_HTTPS) {
            ...
            // 指定request处理函数为requestHandler.userRequestHandler(自定义请求处理器)
        ...
        // 创建websocket服务端，服务器指定为httpProxyServer
        function(callback) {
          wsServerMgr.getWsServer({
            server: self.httpProxyServer,
            connHandler: self.requestHandler.wsHandler // WebSocket.Server connect回调处理函数
          });
      ...
      if (tmpWebServer) {
        ...
        tmpWebServer.close(error => {
          if (error) {
            console.error(error);
            logUtil.printLog(
              `proxy web server close FAILED: ${error.message}`,
              logUtil.T_ERR
            );
          } else {
            logUtil.printLog(
              `proxy web server closed at ${this.proxyHostName} : ${this.webPort}`
            );
          }
