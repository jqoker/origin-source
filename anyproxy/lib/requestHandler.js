          ...
          new Promise((fulfill, rejectParsing) => {
            if (resDataStream) {
              ...
              const refactContentEncoding = () => {
                if (contentEncoding) {
                  resHeader[
                    "x-anyproxy-origin-content-encoding"
                  ] = contentEncoding;
...
function getWsReqInfo(wsClient) {
  ...
  const isEncript =
    true && upgradeReq.connection && upgradeReq.connection.encrypted;
  return {
    hostName: hostName,
    port: port,
    path: path,
...
function getUserReqHandler(userRule, recorder) {
  ...
  return function(req, userRes) {
    ...
    */
    ...
    // rawHeaders中的key,value均在同一个数组中，偶数为key, 奇数为value
     ...
     * 向客户端发送最终响应，响应数据来源于代理向被代理服务器请求得到
    ...
    const sendFinalResponse = finalResponseData => {
      ...
      if (global._throttle) {
        ...
        // 不节流情况，如是可读流，则流向userRes
        // 否则，使用userRes.end方法发送数据并结束
        if (responseBody instanceof CommonReadableStream) {
    ...
    co(fetchReqData)
      ...
      .then(() => {
        ...
        if (recorder) {
          ...
          // 添加记录并返回相应id，用于更新特定记录, recorder信息存入nedb数据库，以便后续通过
          // 请求查看recorder详细信息（比如点击webinterface中的单条请求记录后展现的详情信息，便是通过请求接口获取的）
          resourceInfoId = recorder.appendRecord(resourceInfo);
        }

        // 更新resourceInfo，添加reqBody
        try {
      ...
      .then(
        co.wrap(function*() {
          ...
          // 取出混入用户自定义的请求参数
          // requestOptions为http.request(options)中的入参
      ...
      .then(
        co.wrap(function*(userConfig) {
          // 如果自定义代码中返回了response, 则直接返回响应给client
...
function getWsHandler(userRule, recorder, wsClient) {
  ...
  try {
    ...
    wsClient.onclose = event => {
      logUtil.debug(
        `original ws closed with code: ${event.code} and reason: ${event.reason}`
      );
      const targetCloseInfo = getCloseFromOriginEvent(event);
