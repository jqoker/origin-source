  ...
  getServer() {
        ...
        recorder.getDecodedBody(query.id, (err, result) => {
          ...
          const _resDownload = function(isDownload) {
            ...
            res.json({
              ...
              ref: `/downloadBody?id=${
                query.id
              }&download=${isDownload}&raw=${!isDownload}`
            });
          };
    ...
    app.get("/fetchReqBody", (req, res) => {
      ...
      if (query && query.id) {
        recorder.getSingleRecord(query.id, (err, doc) => {
          ...
          res.setHeader(
            "Content-disposition",
            `attachment; filename=request_${query.id}_body.txt`
          );
    ...
    app.get("/fetchCrtFile", (req, res) => {
      ...
      if (_crtFilePath) {
        ...
        res.setHeader(
          "Content-Disposition",
          'attachment; filename="rootCA.crt"'
        );
        res.end(fs.readFileSync(_crtFilePath, { encoding: null }));
      } else {
        res.setHeader("Content-Type", "text/html");
        res.end(
          "can not file rootCA ,plase use <strong>anyproxy --root</strong> to generate one"
        );
      }
    });
    ...
    app.get("/qr", (req, res) => {
      ...
      const resDom = '<a href="__url"> __img <br> click or scan qr code to start client </a>'
        .replace(/__url/, targetUrl)
        .replace(/__img/, qrImageTag);
      res.setHeader("Content-Type", "text/html");
      res.end(resDom);
    });
    ...
    app.use((req, res, next) => {
      const indexTpl = fs.readFileSync(path.join(staticDir, "/index.html"), {
          encoding: "utf8"
        }),
        opt = {
          rule: ruleSummary || "",
