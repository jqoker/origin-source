...
class WsListener extends React.Component {
  ...
  // websocket 接收消息回调函数，用子线程处理接收到的消息
  onWsMessage(event) {
    try {
      // event.data接收到的数据信息
  ...
  componentDidMount() {
    // 监听RecordWorker，message回调，dispatch数据到store
      ...
      switch (data.type) {
        case 'updateData': {
          if (data.shouldUpdateRecord) {
            this.props.dispatch(updateWholeRequest(data.recordList)); // 更新请求列表
          }
          break;
        }
